 ...
 * 1. 调用后端获取授权 URL
 * 2. 打开授权页面
 * 3. 用户授权后，GitHub 会回调到 /github-callback 页面
 * 4. 在回调页面处理 code 和 state，完成登录
 */
const handleGithubLogin = async () => {
  try {
    // 1. 获取授权 URL
    const { data } = await authStore.getGithubAuthUrl()
    
    // 2. 保存 state 到 localStorage，用于回调时验证
    localStorage.setItem('github_oauth_state', data.state)
    
    // 3. 保存回调页面地址（授权完成后跳转回登录页）
